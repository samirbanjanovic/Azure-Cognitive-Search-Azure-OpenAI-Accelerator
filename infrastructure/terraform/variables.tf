variable "unique" {
  type = bool
  default = true
  description = "Optional. Default is true. A random suffix will is appended to certain resources names to ensure uniqueness."
}
...
variable "azureSearchPartitionCount" {
  type        = number
  default     = 1
  description = "Optional, defaults to 1. Partitions allow for scaling of document count as well as faster indexing by sharding your index over multiple search units. Valid values are from 1 to 12 partitions."
  validation {
    condition     = var.azureSearchPartitionCount >= 1 && var.azureSearchPartitionCount <= 12
    error_message = "Must be a value between 1 and 12"
  }  
}
